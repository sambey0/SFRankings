...
def fetch_professors_by_school(school_id):
    data = {
        ...
        """,
        "variables": {
            "count": 1000,  # Adjust the number of results per page
            "cursor": "",
            "query": {
                "schoolID": school_id,
            }
...
with open('all_schools_data.csv', 'r', encoding='utf-8') as csvfile:
    ...
    # Loop through each school and fetch professors
    for row in reader:
        ...
        if school_id:
            ...
            print(f"Skipping row without school ID: {row}")

# Saving the professors data into CSV
with open('professors_data.csv', 'w', newline='', encoding='utf-8') as file:
...
print(f"All professors data has been saved to 'professors_data.csv'.")
